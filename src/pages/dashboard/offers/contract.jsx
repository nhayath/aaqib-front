...
export default function Offers({ token, data, cond }) {
    ...
    useEffect(() => {
        if (isClient) {
            fetchData(filters, token).then((newOffers) => {
                router.replace(`/dashboard/offers/contract?${filters}`);
                setOffers(newOffers);
            });
        }
    }, [filters, isClient]);
    ...
    return (
        <>
            ...
            />
            <br />
            <OffersList offers={offers} cond={cond} />
        </>
    );
}
...
export async function getServerSideProps({ req, query }) {
    ...
    let url =
        `${process.env.API_URL}/offers/all?dealType=contract&` + args.join("&");
    // console.log(token);
...
async function fetchData(filters, token) {
    try {
        let res = await fetch(
            `${process.env.API_URL}/offers/all?${filters}&aj=1`,
            {
                Accept: "application/json",
                headers: { Authorization: `Bearer ${token}` },
            }
